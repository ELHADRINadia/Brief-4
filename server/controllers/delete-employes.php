<?php
    ...
    function delete_employe()
    {
        ...
        if (not_empty([$id])) {
            ...
            $sql_get_image = mysqli_query($conn,"SELECT image FROM employé  WHERE id='$id'");  
            $result = mysqli_fetch_all($sql_get_image,MYSQLI_ASSOC);

            $image = $result[0]['image'];

            $sql = mysqli_query($conn,"DELETE FROM employé  WHERE id='$id'");  
            ...
            $response = ["status" => "failed","message" => "Please provide employé id!"];
            return $response;
        }
    }
?>
